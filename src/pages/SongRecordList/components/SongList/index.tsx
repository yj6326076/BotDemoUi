...
import { CaretUpFilled, CaretDownFilled, DeleteFilled } from '@ant-design/icons';
...
const DialogTable: React.FC = () => {
  ...
  const cellOperation = (values: unknown, row: any[]): React.ReactNode => {
    ...
    return (
      <div>
        <CaretUpFilled onClick={() => operationCallback({ actionType: 'edit', dataSource: record })} />
        &nbsp;&nbsp;
        <CaretDownFilled type="primary" onClick={() => handleDelete(record)} />
        &nbsp;&nbsp;
        <DeleteFilled type="primary" onClick={() => operationCallback({ actionType: 'preview', dataSource: record })} />
      </div>
    );
  };
